...
func init() {
	...
	flags.BoolVar(
		&args.createClusterAdmin,
		"create-cluster-admin",
		false,
		"Create cluster admin once the cluster is installed",
	)
...
func run(cmd *cobra.Command, _ []string) {
	...
	createClusterAdmin := args.createClusterAdmin
	clusterAdminUser := strings.Trim(args.clusterAdminUser, " \t")
	clusterAdminPassword := strings.Trim(args.clusterAdminPassword, " \t")
	autogeneratedAdminCreds := false

	if !isHostedCP {
		if createClusterAdmin {
			//if flag to create admin is set
			//then validate provided user/passwd
			// or autogenerate them
			...
			if clusterAdminPassword != "" {
				...
				autogeneratedAdminCreds = true
				clusterAdminPassword, err = admin.GenerateRandomPassword(23)
				if err != nil {
					r.Reporter.Errorf("Failed to autogenerate a random password %s", err)
					os.Exit(1)
				}
			}
		}
		//if Admin Creds are not setup gather interactively
		if (clusterAdminUser == "" || clusterAdminPassword == "") && interactive.Enabled() {
			createClusterAdmin, err = interactive.GetBool(interactive.Input{
				Question: "Create Cluster Admin",
				Default:  false,
				Required: true,
			})
			if err != nil {
	...
	if !output.HasFlag() || r.Reporter.IsTerminal() {
		...
		if createClusterAdmin {
			...
			r.Reporter.Infof("Cluster Admin user account will be added to the cluster once the install is complete." +
				"It may take several minutes for this user account to become active.")

			//display creds only if they were generated by rosa cli
			if autogeneratedAdminCreds {
				r.Reporter.Infof("Please securely store the following generated cluster admin credentials.\n " +
					"If you lose the credentials you can delete and recreate the admin user." +
					"See 'rosa create|delete admin --help' for more information.")
				r.Reporter.Infof("To login to the cluster as admin, use the username: %s and password: %s",
					clusterAdminUser, clusterAdminPassword)
			}
